...
PHASE 1 — GLOBAL ANALYSIS (один раз):
  ...
  [G4] Content Inventory (агент 11) — для КАЖДОГО слайда выписать весь контент
       (заголовки, буллеты, числа/KPI, подписи, ячейки таблиц, картинки; для схем —
       6 категорий F/N/T/W/P/L). Это эталон для проверки паритета ниже.
...
PHASE 2 — PER-SLIDE DESIGN LOOP (для каждого слайда независимо):
  ...
  │
  │      [S4] Local validate — slot fits, overflow strategies + **Content Parity
  │           (агент 11): весь контент из [G4] присутствует? ничего не потеряно/
  │           не ужато до исчезновения. При overflow — стратегия, НЕ обрезка.**
  │
...
PHASE 3 — GLOBAL VERIFICATION (один раз в конце):
  [V0] Content Parity (агент 11) — сверить output ↔ инвентарь [G4] по ВСЕМ слайдам:
       контент перенесён в точности (0 потерь). Для схем — legend⇒referent + счётчики.
...
{"slide_type": "flow_diagram_native", "dark": false, "flow": {
  ...
  "grid_snap": true,                       // v2.5.1 опц., default true: snap всех
  "auto_align": true,                      //   блоков/таблиц к сетке + выравнивание
  "align_tol": 12,                         //   близких краёв к общим осям
  "blocks": [
    {"id": "b1", "x": 175, "y": 180, "w": 235, "h": 50,
     "lines": ["Title"], "font_sizes": [13], "bolds": [true]}
  ],
  "tables": [                              // ⭐ v2.5.1 таблица ВНУТРИ схемы (всегда с границами)
    {"id": "rt1", "x": 470, "y": 182, "w": 330,
     "title": "Route table 1", "title_fill": "#8A8A8A",    // опц. бэнд-заголовок
     "headers": ["Destination", "Next Hop", "Route Type"], // опц. строка заголовков
     "rows": [["0.0.0.0/0", "VPC 3 attachment", "Static route"]],
     "col_fracs": [0.32, 0.36, 0.32],      // опц. доли ширины колонок
     "font_pt": 9, "title_font_pt": 11, "header_font_pt": 9,
     "row_h": 22, "title_h": 24, "header_h": 20}            // МИНИМУМЫ (авто-рост под перенос)
  ],
  "arrows": [
    {"from": "b1", "to": "b2", "side": "right"},
    {"from": "rt1", "to": "b1"},           // стрелки якорятся и к таблицам по id
...
### Canonical правило v2.5.1 — сетка, выравнивание, таблицы-в-схеме (2026-06-05)

Хард-схемы (десятки элементов: VPC-панели + вложенные route-таблицы + иконки +
стрелки + легенда — напр. «Enterprise Router + Cloud Firewall») раньше
разваливались: блоки задавались «на глаз», таблиц как структуры не было.

**1. Сетка (`grid_snap`, default true).** Все координаты блоков и таблиц
привязываются к шагу `GRID_STEP=8 px` (`_snap_rect`) — ровный ритм без
суб-пиксельного дребезга.

**2. Выравнивание осей (`auto_align`, default true, `align_tol=12`).**
Пост-проход `align_rects` кластеризует близкие **left/top** и стягивает их к общей
вертикали/горизонтали — колонки и ряды читаются как сетка. **Только позиции, без
ресайза** (right/bottom-кластеризация «распирала» таблицы — отключена 2026-06-05).
Идемпотентно. Элемент-исключение: `"no_align": true`.
⚠️ **Для МНОГОЗОННЫХ схем (несколько независимых блоков-зон) — `auto_align=false`**:
глобальный кластер стянет таблицы РАЗНЫХ зон к общим осям и оторвёт от подписей.
auto_align хорош для линейных/одно-зонных раскладок; multi-zone — координаты
вручную + `grid_snap`.

**3. Таблица внутри схемы — ВСЕГДА с границами** (`flow.tables[]`,
`add_grid_table`). Требование: **если таблица лежит на слайде вместе с другими
объектами (текст/иконки/стрелки/панели — т.е. внутри схемы) — у неё должны быть
очерчены края** (полная сетка тонких серых границ `#C8C8C8`). **Если на слайде
ТОЛЬКО таблица — стиль не меняем**: это отдельный путь `table_native` (zebra,
без границ), его не трогаем. Внутри `flow_diagram_native` таблица по определению
среди других элементов → границы рисуются всегда.
- Структура: опц. `title` (бэнд-заголовок во всю ширину, цвет текста авто по
  яркости `title_fill`) + опц. `headers` (строка заголовков колонок) + `rows`.
- `col_fracs` — доли ширины колонок (иначе равные). `row_h/title_h/header_h` —
  это **минимумы**: высота строки авто-растёт под перенос текста (узкие колонки
  не обрезают контент). Стрелки якорятся к таблице по `id`.
- **Текст ячеек — по ЛЕВОМУ + ВЕРХНЕМУ краю** (canonical, в т.ч. title/headers).

**Доп. (2026-06-05):**
- **Пунктир спокойный** (`add_dashed_rect`): тонкая 0.75pt + крупный равномерный
  штрих (`custDash`, по умолч. `dash_pt=5`, `gap_pt=4`) — не «рябит». Группы
  (`groups`) тоже снапятся к сетке, рамки не «съезжают» относительно контента.
- **Цвет из конфига**: `arrows[].color` и `labels[].color` (hex) пробрасываются.
- **Палитра: серый-доминант + зелёные акценты.** Заливка панелей/бэндов — серая;
  другой цвет — ТОЛЬКО для смысловой дифференциации путей/сущностей по легенде
  (напр. зелёная «associate attachment», красный ✕ «blocked»). Не раскрашивать
  сущности ради декора.

**Маршрутизация (Slide Classifier):** таблица + другой контент на одном слайде
→ `flow_diagram_native` с `tables[]` (границы). Одиночная таблица → `table_native`
(zebra). Сомнение про merged/irregular → anti-distortion stop+ask.
...
- регулярная таблица ≥3×3 с шапкой, БЕЗ merged cells, **одна на слайде** → `table_native` ⭐ (v1.8+, zebra без границ)
- таблица(ы) **вместе с другим контентом** (текст/иконки/стрелки/панели — схема) → `flow_diagram_native` с `tables[]` ⭐ (v2.5.1, **с границами**)
...
**⭐ v2.5.1 — Анализатор полноты КОНТЕНТА (`agents/11-schema-completeness-analyzer.md`)
— для ВСЕХ слайдов:** обязательный шаг pipeline ([G4]/[S4]/[V0]). Для любого слайда
гарантирует, что весь контент драфта (заголовки, буллеты, числа, ячейки) перенесён
1:1 без потерь (overflow → стратегия, не обрезка). Для СХЕМ — полный режим: инвентарь
по 6 категориям (frames/nodes/tables/words/paths/legend) → перекрёстные правила
(**каждая запись легенды ОБЯЗАНА иметь referent в теле**; cell-mention⇒node; внутренние
потоки зон) → перенос ВСЕГО → расстановка по сетке + перекраска → **parity-check** →
visual-diff. Девиз: не упрощать — перенести всё, расставить, перекрасить.

**⭐ v2.5.1 — Свод правил ХАРД-СХЕМ:** `references/flow_diagrams/HARD-SCHEMA-RULES.md`
(полнота → сетка/оси → слои фреймов → анти-наезд → цвет → визуальная сверка). Читать
перед вёрсткой нагруженных схем (зоны+вложенные таблицы+потоки). Эталон-реализация:
`tests/hard_schema_build.py` (инвентарь в шапке + `_parity_check()` в конце).
...
- **⭐ v2.5.1 — СЛОИ ФРЕЙМОВ (иерархия границ областей, бренд-спека 2026-06-05):**
  области выделяются ПРЕИМУЩЕСТВЕННО ЗАЛИВКОЙ. Заливки **ЧЕРЕДУЮТСЯ** по
  вложенности, начиная ОТ ФОНА. Слайд белый = Background — **отдельный серый
  фон-стейдж НЕ добавлять** (даёт серые «колонки» в зазорах между зонами).
  Чередование от белого слайда:
  - **Зона** (внешний фрейм, напр. VPC, Enterprise router): `style="panel"`, fill
    **Gray 100** `#F2F2F2` + `bottom_stroke={"color":"#D3D3D3","w_pt":1.0}` (нижний
    штрих graphite-50, НЕ рамка).
  - **Вложенная группа** (напр. security group): `style="panel"`, fill **White**
    (чередование заливки на серой зоне).
  - **Глубже** (напр. subnet): пунктир `#D3D3D3` (`add_dashed_rect`, тире=пробел=2pt,
    0.75pt). **Максимум ОДИН пунктирный слой** (несколько боксов на ОДНОМ уровне — ок).
  - **Компонент/узел** (ECS, Firewall, карточка): `blocks[].fill="#F2F2F2"` +
    `bottom_stroke={"color":"#26D07C","w_pt":1.0}` — зелёный нижний штрих как акцент.
  - Нижние штрихи — на СЕРЫХ фреймах (зона, узел); белые вложенные группы — без штриха.
  - ⚠️ Не инвертировать порядок и не делать серый фон-стейдж (обе ошибки наблюдались).
  - Цвет штрихов фреймов = `#D3D3D3` (graphite-50), НЕ `#434343`. Стрелки-трафик
    остаются graphite `#434343` (это контент). Минимизируй число обводок в
    нагруженных схемах — заливка вместо рамок. Хелперы: `add_bottom_stroke`,
    `add_filled_panel(bottom_stroke=)`, `add_block(bottom_stroke=)`, `add_solid_rect`.
...
### Spec-driven дисциплина (Point 10, из дизайн-аудита)
- **plan.json — исчерпывающий контракт.** Рендер НЕ угадывает: цвет, кегль, отступ,
  donor — всё берётся из plan.json/токенов. Если рендеру пришлось бы «угадать» —
  это провал извлечения/планирования, а не задача рендера.
- **Gate перед сборкой:** `validate_plan.py` достраивает canonical-оверрайды и ловит
  overflow ДО build (а не «на глаз» в рендере). Не строй по неполному плану.
- **Один слайд = одна мысль.** Если конфиг слайда распух (много блоков/текста) —
  это сигнал split (см. overflow strategy), а не «впихнуть и уменьшить кегль».
- **Идемпотентность/безопасность:** правки бренда (enforce_canonical, normalizer) —
  только снимают/снапят к токенам, не ломают раскладку; повторный прогон не портит.
