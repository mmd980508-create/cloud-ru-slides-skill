...
## v2.5.1 — 2026-06-05 — хард-схемы: сетка + выравнивание + таблицы-в-схеме с границами

**Не работало:** нагруженные схемы (десятки элементов — VPC-панели + вложенные
route-таблицы + иконки + стрелки + легенда, напр. «Enterprise Router + Cloud
Firewall») разваливались. Блоки задавались «на глаз» (нет сетки/выравнивания),
а структуры «таблица» внутри схемы не было вовсе — приходилось собирать из блоков
без границ.

**Сделали** (`scripts/flow_renderer.py`):
1. **Сетка** — `GRID_STEP=8`, `_snap_rect`; флаг `flow.grid_snap` (default true).
2. **Выравнивание осей** — `align_rects` + `_cluster_axis`: кластеризация близких
   краёв (L/T/R/B, `align_tol=12`) к общим вертикалям/горизонталям. Флаг
   `flow.auto_align` (default true), opt-out `"no_align":true`. Идемпотентно —
   preset/grid и baseline не страдают (snap/align идут ПОСЛЕ `if preset: return`).
3. **Таблица внутри схемы — всегда с границами** — `flow.tables[]` + `add_grid_table`:
   опц. `title` (бэнд, цвет текста авто по яркости), опц. `headers`, `rows`,
   `col_fracs`. Тонкие серые границы `#C8C8C8` на каждой ячейке (полная сетка).
   **Высота строк авто-растёт под перенос** (`_row_need_h`) — узкие колонки не
   режут текст. Стрелки якорятся к таблице по `id`.

**Правило (требование пользователя):** таблица + другой контент на слайде →
`flow_diagram_native` с `tables[]` (границы). Одна таблица на слайде → `table_native`
(zebra, без границ — не трогаем).

**Тест:** `tests/hard_schema_build.py` → `tests/png_hard/slide-01.png` — полная
реконструкция хард-схемы Enterprise Router + Cloud Firewall (3 VPC-зоны, 6
route-таблиц с границами, стрелки, легенда). Визуально сверено: текст не вылезает,
таблицы выровнены, границы на месте. Регрессия проверена (preset/grid/manual паттерны).
Ограничения: иконки (cloud/ECS/firewall) — текст-плейсхолдеры (не brand-SVG);
диагональные «associate attachment» линии оригинала заменены ортогональными.

**Доработка по фидбеку (2026-06-05, та же v2.5.1):**
1. **Текст ячеек таблицы — по ЛЕВОМУ + ВЕРХНЕМУ краю** (`_table_cell` anchor=TOP;
   `add_grid_table` defaults `title_align/header_align="left"`). Раньше заголовки
   были center + vertical middle.
2. **Спокойный пунктир** — `add_dashed_rect` перешёл с preset `dash` на `custDash`
   (тонкая 0.75pt + крупный равномерный штрих `dash_pt=5/gap_pt=4`): не «рябит».
3. **Рамки/группы на сетке** — `groups` тоже проходят `_snap_rect` (раньше только
   blocks/tables), пунктирные рамки больше не «съезжают» относительно контента.
   Подпись рамки — по левому краю с инсетом 8px.
4. **Цвет стрелок и меток из конфига** — `arrows[].color` и `labels[].color`
   (hex) теперь пробрасываются (`add_orthogonal_arrow`/`add_arrow`/`add_label`).
5. **Серый-доминант + зелёные акценты** — в тест-схеме VPC-панели и все бэнды
   route-таблиц серые; «associate attachment» — зелёная линия (акцент+смысл),
   blocked — красный ✕ (единственное смысловое исключение). Цветовую
   дифференциацию несёт легенда (стиль/цвет линий), а не заливка сущностей.

**Фидбек-2 (2026-06-05, «стало хуже» → исправлено):**
- **БАГ: серые плашки-заголовки вдвое выше нужного (42-45px).** Причина —
  `_row_need_h` оценивал перенос ТАЙТЛА (merged во всю ширину) по ширине ПЕРВОЙ
  КОЛОНКИ (~110px) → думал, что «VPC 1 route table» в 2 строки → ×2 высота.
  Фикс: тайтл считается по полной ширине `w` (`cell_ws=[w]`). + ужал формулу
  (line 1.28→1.18, pad 8→5) и поля ячейки (5/4→3/2). Итог: 42px → **19-20px**.
- **Глобальный `auto_align` ломал МНОГОЗОННЫЕ схемы** — стягивал таблицы РАЗНЫХ
  зон к общим осям и отрывал их от подписей (текст «уезжал»). Фиксы: (а)
  `align_rects` теперь ТОЛЬКО позиции (left/top), без ресайза ширины/высоты —
  ресайз через right/bottom «распирал» таблицы; (б) для многозонных схем
  `auto_align=false` (координаты вручную, grid_snap оставляем). Правило: глобальный
  кластер-алайн хорош для ЛИНЕЙНЫХ/одно-зонных схем; для multi-zone — выключать.
- **Зелёный акцент стал виден** — добавлен brand `decor` (↗ зелёные) в нижний-
  правый угол; легенда: зелёная associate-линия + красный ✕.

**Фидбек-3 (2026-06-05, «элементы раскиданы / заголовки не на одном месте»):**
Корень — координаты «на глаз». Перевёл тест-схему на **дисциплину сетки** (см.
переписанный `tests/hard_schema_build.py`):
- **Единая типографика таблиц** — title 10pt / header 9pt / data 9pt ВЕЗДЕ (раньше
  центр 10, бока 9 → плашки «гуляли»). + поля ячейки 10/8/4/3 (текст не липнет).
- **Две «линии чтения» (ряды) с выравниванием по ВЕРХУ** — `T1=246`/`T2=486`:
  таблицы каждого ряда из всех 3 зон ставятся одним верхом → плашки-заголовки
  пиксель-в-пиксель на одной линии (проверено: top 248/488, h 20). Стрелки —
  на фикс. высотах `ARROW1/ARROW2`, горизонтальны.
- **Иконки центрируются по СВОЕЙ таблице** — `add_table` возвращает верт. центр,
  `add_icon` кладёт бокс по нему (единый размер 88×46).
- **Высота таблицы считается зеркалом формулы рендерера** (`table_h` в конфиге:
  `_cell_lines/_row_h`) → центрирование иконок и контроль вписывания точные.
- **Панели-зоны единых габаритов** (верх 146, низ 594; левые h=200).
Вывод-правило: для нагруженных multi-zone схем строить ПО СЕТКЕ (общие baseline
рядов, единые компонент-шаблоны), а не координатами на глаз; высоту таблиц
оценивать той же формулой, что рендерер.

**Фидбек-4 (2026-06-05, «пропали детали / не все закономерности соблюдены»):**
Реконструкция ТЕРЯЛА детали — выкинуты линии associate-attachment, внутренний
поток ECS3→Firewall, blocked ✕, иконки → схема сведена к «панели+таблицы+стрелки».
Симптом: легенда без referent в теле.
- **Добавлен `agents/11-schema-completeness-analyzer.md`** — инвентарь источника
  по 6 категориям (frames/nodes/tables/words/paths/legend) + перекрёстные правила
  (legend⇒referent; cell-mention⇒node; внутренние потоки) + parity-check +
  visual-diff. Девиз: не упрощать — перенести ВСЁ, расставить, перекрасить.
- Вписан в classifier (шаг реконструкции, обязателен) + SKILL.md (canonical v2.5.1).
- **Восстановлено в тест-схеме:** вложенные рамки sg+subnet на ВСЕХ 3 VPC;
  внутренний поток ECS3(eth0)↔Firewall(eth1) (request↓ solid / response↑ dashed) +
  blocked ✕ на сегменте; associate-attachment зелёные линии (referent легенды).
  В `hard_schema_build.py` добавлен `_parity_check()` (печатает счётчики + legend
  ⇒referent OK/BROKEN). Текущий parity: F dashed=7, P assoc=5, legend⇒referent OK.

**Фидбек-5 (2026-06-05, «выравнивание + слипаются рамки» → СЛОИ ФРЕЙМОВ):**
Вложенные пунктирные рамки (panel+sg+subnet, все пунктир) слиплись и шумели.
Пользователь прислал бренд-спеку слоёв. Внедрено:
- **Иерархия границ:** заливка (1) → сплошная 0.75pt `#D3D3D3` (2) → пунктир (3).
  **Пунктир максимум на ОДНОМ слое**, тире=пробел=2pt, 0.75pt, цвет `#D3D3D3` (не #434343).
- **flow_renderer:** `FRAME_STROKE=#D3D3D3`; `add_bottom_stroke()`; `add_filled_panel
  (bottom_stroke=)`; `add_block(bottom_stroke=)`; `add_solid_rect()`; group `style="line"`;
  `add_dashed_rect` default `#D3D3D3` dash 2/gap 2/0.75pt; `DASH_GRAY`→`#D3D3D3`.
- **Схема:** VPC=L2 (Gray100 `#F2F2F2` + нижний штрих `#D3D3D3`); security-group=L3
  (пунктир, единственный слой); subnet=подпись; ECS/Firewall=white + зелёный нижний
  штрих (акцент-компонент). Шум обводок резко снижен, границы не слипаются.
- Документировано: SKILL.md «СЛОИ ФРЕЙМОВ», memory `feedback_frame_layers`, agent 11.
- Регрессия (preset/grid/panel+bottomstroke+node) — зелёная.

**Фидбек-6 (2026-06-05, «гайды не соблюдены» — ИНВЕРСИЯ слоёв):** в фидбеке-5
заливки были инвертированы. Правильно заливки ЧЕРЕДУЮТСЯ: **Background Gray 100 →
L1 White → L2 Gray 100 + нижний штрих → L3 пунктир**. Исправлено в схеме:
- Добавлен серый фон-стейдж (panel `#F2F2F2`, первым в groups).
- VPC-зоны = **White** (L1, было серым); центр Enterprise router = White panel.
- Security group = **Gray 100 + нижний штрих** (L2, было пунктиром).
- Subnet = **пунктир** (L3, было подписью) — единственный пунктирный слой.
- Узлы ECS/Firewall = white + зелёный нижний штрих (без изменений).
Память `feedback_frame_layers` + SKILL.md обновлены с явным предупреждением
«НЕ инвертировать; заливки чередуются». parity: panels=8, dashed=4 (один слой L3).

**Консолидация (2026-06-05, финал сессии):**
- **Свод правил хард-схем** → `references/flow_diagrams/HARD-SCHEMA-RULES.md`
  (порядок: полнота → сетка/оси → слои фреймов → анти-наезд → цвет → сверка).
  Собраны ВСЕ замечания сессии: единая типографика таблиц, выравнивание по осям
  (multi-zone auto_align off), слои фреймов (заливка→#D3D3D3→пунктир, 1 слой пунктира),
  ОДНА логика серых рамок (только VPC-зоны; router без рамки), анти-наезд (детектор,
  иконка-таблица зазор, колонки под контент), цвет (серый+зелёный/красный по легенде).
- **Анализатор полноты расширен на ВСЕ слайды** (agent 11): добавлен лёгкий режим
  паритета для обычных слайдов (буллеты/числа/ячейки 1:1, overflow→стратегия не
  обрезка) + полный режим для схем. Вшит в workflow SKILL.md: [G4] Content Inventory
  → [S4] Content Parity → [V0] global Content Parity.
- Финальная хард-схема: gray-фреймы = только VPC; центр без рамки; наездов 0
  (детектор чист); оси выровнены. Сборка/импорт зелёные.

## v2.5 — 2026-06-02 — РЕЛИЗ: десятка дизайн-аудита закрыта (Point 10: spec-driven дисциплина)

(Бывш. v2.4.10 — переименовано в v2.5 как релизный рубеж: все 10 пунктов
дизайн-аудита внедрены, v2.4.2 … v2.4.9 + этот.)

Из дизайн-аудита п.10. Честно: ядро УЖЕ встроено (plan.json = исчерпывающий спек,
`validate_plan.py` гейтит его до build). Формализовали принцип в SKILL.md «Принципы
работы»: рендер не угадывает (всё из plan.json/токенов); gate перед сборкой; один
слайд = одна мысль (распух → split, не «впихнуть и уменьшить кегль»); идемпотентность
бренд-правок (enforce/normalizer снимают/снапят, не ломают раскладку).

**Десятка из дизайн-аудита закрыта (v2.4.2 … v2.4.10).** Визуально-меняющие: п.6
(трекинг цифр), п.7 (KPI-композиция). Остальные — QA/проверки/принципы в одном
`brand_guardian` + DESIGN.md/агенты, без раздувания сущностей.

## v2.4.9 — 2026-06-02 — Point 9: принципы ремесла (Emil/impeccable) в LLM-критику

Из дизайн-аудита п.9. Не код — операционные принципы для авто-инжектируемого
`DESIGN.md` (агенты 04 Layout Designer, 10 Visual Verifier). Моушен-советы
web-источников отброшены (статический медиум).

### ✅ Что добавилось
- **`DESIGN.md` §6b** — принципы: невидимые-детали-суммируются (выравнивание/опт.центр),
  сдержанность=иерархия (один акцент), асимметричный акцент, оптические правки,
  ≤4 элемента в группе; + чек-шаги перед READY: **squint-тест**, **cold-review**, **AI-slop тест**.
- **`agents/10-llm-visual-verifier.md`** — добавлены пункты F.14–17 (squint/cold/slop/
  сдержанность) в чек-лист верификации.

Цель — усилить дизайнерское рассуждение LLM на финале (не код-метрики).

## v2.4.8 — 2026-06-02 — Point 8: copy-стоп-лист (штампы) в brand_guardian + агент 07

Из дизайн-аудита п.8. По решению user — в `brand_guardian` (+ правило в LLM-агента 07).

### ✅ Что добавилось
- **`brand_guardian.check_copy(slide)`** + `COPY_CLICHES` — ловит штампы/филлер
  («в современном мире», «более того», «стоит отметить», «в заключение», «бесшовный»,
  «синергия», «раскрыть потенциал», конструкция «не просто X — это/а Y») → warnings
  (категория Content). Список ТЕСНЫЙ — не трогает легитимную лексику Cloud.ru
  («надёжный», «облако», «инфраструктура», «безопасность»).
- **`agents/07-copy-editor.md`** — тот же денилист как правило для LLM-редактора
  (штамп → предложить замену в warnings, не молча переписывать смысл).

### Проверено
Слайд с 4 штампованными фразами → поймано 7 штампов; «надёжность/облако/безопасно»
НЕ флагаются (без ложных срабатываний).

## v2.4.7 — 2026-06-02 — Point 7 финал: card-KPI = «вариант 2» (уверенная композиция)

После ревью user'а («не секси, композиция неуверенная») собрали 3 уверенных
варианта (`tests/point7_composition/kpi_variants.py`): 1 герой-цифра, 2 карточки
во всю высоту, 3 цифры на базовой линии. **Выбран вариант 2.**

### ✅ render_kpi_cards = вариант 2
Карточки ВО ВСЮ ВЫСОТУ (CARD_Y=125, H=515), ярлык-таб СВЕРХУ (UPPERCASE; на
зелёной акцент-карточке — графит, на серых — `TEXT_GRAY` #5C5C5C), крупная цифра
У НИЗА (bottom-left), `%` меньшим кеглем в строке (`_add_number_box`), трекинг §6.
Акцентная карточка — зелёная. `TEXT_GRAY` константа добавлена.
Варианты 1 и 3 отброшены (kpi_variants.py оставлен как запись эксперимента).

## v2.4.6 — 2026-06-02 — Point 7: композиция KPI (акцент, начертание, edge-выравнивание)

Из дизайн-аудита п.7 (композиция архетипов). По итогу обсуждения с user —
сфокусировались на KPI (numbered_columns не трогаем: номера-у-низа могут быть
намеренным якорем из эталона, не баг).

### ✅ Правила KPI (user-decision 2026-06-02)
- **Крупные цифры — Regular** (bold=False), не SemiBold. Код уже был верный;
  «тяжесть» в превью — артефакт фоллбэк-шрифта (SB Sans тут не установлен).
- **У «голых» крупных цифр акцента НЕТ** (ни плашки, ни полосы) — убран зелёный блок.
- **Зелёная акцент-плашка — ТОЛЬКО в card-layout** (все цифры в карточках):
  новый `kpi.layout="cards"` → каждая цифра в карточке #F2F2F2, акцентная карточка
  ЗЕЛЁНАЯ, цифра графит (white-on-green запрещён, графит на зелёном ок).
- **Выравнивание по КРАЯМ, не по центру** (редакторский стиль): голый вариант —
  цифры по левому краю сетки; карточки — цифра верх-лево, подпись низ-лево.
- **`%` в строке** с цифрой (меньшим кеглем) — новый `_add_number_box`.

### Код
`kpi_renderer.py`: `_add_number_box` (цифра+% , left/edge align, трекинг §6);
`render_kpi_cards` (card-layout); голый `render_kpi` — left-align, без акцента.
`GRAY` константа добавлена.

### Тест
`tests/point7_composition/kpi_cards_demo.py` (голый vs card), `kpi_accent_options.py`
(варианты акцента — для выбора).

## v2.4.5 — 2026-06-02 — Point 6: типографика (отрицательный трекинг крупных цифр)

Из дизайн-аудита п.6. Честная оценка для PPTX-медиума:
- **tabular-nums** — невозможно в PPTX (нет OOXML-атрибута для OpenType-фич); нашим
  KPI и не нужно (каждая цифра в своём центрированном боксе). Пропущено.
- **Шаг шкалы ≥1.25** — уже закрыт `muddy_hierarchy` (v2.4.4).
- **Трекинг заголовков** — оставлен 0 (по брендбуку; user-decision 2026-06-02).

### ✅ Внедрено (брендбук-совместимо, видимо)
Брендбук §6 требует у крупных цифр ОТРИЦАТЕЛЬНЫЙ трекинг, а `kpi_renderer` его не
ставил. Добавлен параметр `spc` в `_add_text_box` (letter-spacing, 1/100 pt, может
быть < 0) и константа `BIG_NUMBER_SPC = -400` — применяется к крупным KPI-цифрам.
Divider-цифры из шаблона НЕ трогаем (свёрстаны дизайнерами Cloud.ru).

### Проверено
`tests/point6_typography/kpi_demo.py` — KPI-слайд 0 vs -400: цифры в «-400» заметно
собраннее/плотнее, как просит §6. CAPS-трекинг демо (`build.py`) — для выбора уровня.

## v2.4.4 — 2026-06-02 — Point 5: anti-slop линтер в brand_guardian

Из дизайн-аудита п.5 (anti-slop / AI-tells). По решению user — в `brand_guardian`
(он же и есть быстрый не-LLM чекер до дорогого визуального прохода).

### ✅ check_slop(slide) — ловит то, что не ловили остальные чеки
- **card_in_card** — крупный filled-прямоугольник внутри другого (вложенные боксы;
  чипы/маркеры исключены порогом размера 150×90px; full-bleed фон исключён).
- **rainbow_accents** — ≥3 разных акцентных цвета заливок (green/yellow/purple/blue).
- **gradient_text** — gradFill в run (градиентный текст).
- **muddy_hierarchy** — >6 разных кеглей на слайде.
Все → warnings; замаплены в категории скоринга (Composition/Color/Typography) и оси.

### Проверено
- демо-слайд (вложенные карточки + 4 радужных блока + 10 кеглей) → пойманы все 3;
- реальный сгенерённый деком → 0 ложных срабатываний.

### Не вошло (контент-зависимое, по геометрии ненадёжно)
иконка-в-скруглённом-квадрате над каждым заголовком, повторяющиеся 01/02/03-маркеры,
крошечный CAPS-eyebrow над каждой секцией — оставлено LLM-визуальному проходу.

### Тест
`tests/point5_slop/build.py` — намеренно слоповый слайд + печать находок линтера.
